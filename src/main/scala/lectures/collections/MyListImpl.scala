...
object MyListImpl extends App {
  ...
  case class MyList(data: List[Int]) {
    ...
    def map(f: Int => Int): MyList = MyList(data.map(f))
//
...
//
    def filter(f: Int => Boolean): MyList = MyList(data.filter(f))
  }
  ...
  println(MyList(List(1, 2, 3, 4, 5, 6)).foldLeft(0)(tpl => tpl._1 + tpl._2))
